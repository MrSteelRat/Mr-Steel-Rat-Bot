...
void Database::createUser(int userId, const std::string& username) {
    const char* insertSQL = "INSERT OR REPLACE INTO users (user_id, username) VALUES (?, ?);";
    sqlite3_stmt* stmt;
    ...
    if (sqlite3_prepare_v2(db, insertSQL, -1, &stmt, nullptr) == SQLITE_OK) {
        ...
        if (sqlite3_step(stmt) != SQLITE_DONE) {
            std::cerr << "Cannot execute insert statement: " << sqlite3_errmsg(db) << std::endl;
        }

        sqlite3_finalize(stmt);
    } else {
        std::cerr << "Cannot prepare insert statement: " << sqlite3_errmsg(db) << std::endl;
    }
}
